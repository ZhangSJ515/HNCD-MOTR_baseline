...
def parse_option():
    ...
    # Proposals toggle: presence of the flag enables proposals, absence disables.
    parser.add_argument("--use-proposals", action="store_true", default=False,
                        help="Enable USE_PROPOSALS (MOTRv2-style proposals from a detector). "
                             "Omit the flag to run without proposals.")

    # Architecture switch: which track-update / first-decoder-layer regime to use.
    # 'memotr' = TIM long-term memory + merge_det_track first-layer trick (current default).
    # 'motrv2' = QIMv2 self-attention + merge_det_track_layer=0.
    parser.add_argument("--arch", type=str, choices=["memotr", "motrv2"], default=None,
