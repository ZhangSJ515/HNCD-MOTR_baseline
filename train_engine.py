...
def train_one_epoch(model: HNCD, train_states: dict, max_norm: float,
    ...
    for i, batch in enumerate(dataloader):
        ...
        for frame_idx in range(len(batch["imgs"][0])):
            ...
            # 如果配置不使用 proposals，传递给模型后模型内部会自行处理
            # 但是 criterion 需要正确的 num_proposals，为了稳妥我们判断一下
            use_proposals = getattr(get_model(model), 'use_proposals', True)
            num_proposals = len(proposals) if use_proposals else 0
