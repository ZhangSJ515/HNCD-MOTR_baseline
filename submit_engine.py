...
def submit(config: dict):
    ...
    # NOTE proposals
    # Default to training config's USE_PROPOSALS (how the model was trained).
    # Allow presence of "--use-proposals" on the command line to force-enable
    # (the flag is now an action='store_true', no value follows it).
    use_proposals = train_config.get("USE_PROPOSALS", True)
    import sys
    if "--use-proposals" in sys.argv:
        use_proposals = True
    train_config["USE_PROPOSALS"] = use_proposals

    det_db = None
    if use_proposals and config.get('DET_DB'):
